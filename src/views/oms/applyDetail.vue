    ...
    <el-card shadow="never">
      ...
      <el-table
        ...
        <el-table-column label="属性" width="180" align="center">
          <template slot-scope="scope">{{scope.row.productAttr == null?"暂无":"暂无"}}</template>
        </el-table-column>
      ...
      <div style="float:right;margin-top:15px;margin-bottom:15px">
        ...
        <span class="font-title-medium color-danger">￥{{orderReturnApply.returnAmount}}</span>
      </div>
    ...
    <el-card shadow="never" class="standard-margin">
      ...
      <div class="form-container-border">
        ...
        <el-row>
          ...
          <el-col class="form-border font-small" :span="18">{{orderReturnApply.createTime}}</el-col>
        </el-row>
        <el-row>
      ...
      <div v-show="orderReturnApply.receivingStatus==1"  class="form-container-border">
        <el-row>
        ...
        <el-row v-if="updateStatusParam.companyAddressId != null">
      ...
      <div class="form-container-border" v-show="orderReturnApply.status===2 && orderReturnApply.receivingStatus==1">
        <el-row>
      ...
      <div style="margin-top:15px;text-align: center" v-show="orderReturnApply.status===0 && orderReturnApply.receivingStatus == 0">
        <el-button type="primary" size="small"  @click="handleUpdateStatus(1)">同意退款</el-button>
        <el-button type="danger" size="small" @click="handleUpdateStatus(2)">拒绝退款</el-button>
      </div>

      <div style="margin-top:15px;text-align: center" v-show="orderReturnApply.status===0 && orderReturnApply.receivingStatus == 1">
        <el-button type="primary" size="small"  @click="handleUpdateStatus(3)">同意退货</el-button>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.getDetail();
      // if(this.orderReturnApply.receivingStatus == 1){
    ...
    methods: {
      ...
      getDetail() {
        this.$axios.get('/oms/order/getDealOrder/'+this.id).then(res=>{
          this.orderReturnApply = res.data.data
          this.product.push(this.orderReturnApply)
        })
      },

      //获得收货地址
      getCompanyAddressList() {
        this.$axios.get('/oms/order/getAddress').then(res => {
          this.companyAddressList = res.data.data;
          for (let i = 0; i < this.companyAddressList.length; i++) {
      ...
      //status:(1同意退款 3同意退货 2拒绝 4确认收货)
      handleUpdateStatus(status){
        this.updateStatusParam["code"]=status
        this.updateStatusParam["id"]=this.id
        this.updateStatusParam["userId"]=this.$store.state.menu.userId
        ...
        this.$confirm('是否要进行此操作?', '提示', {
          ...
          this.$axios.post('/oms/order/updateOrderStatus',this.updateStatusParam).then(res=>{
